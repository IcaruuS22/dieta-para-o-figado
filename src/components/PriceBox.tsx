...
export const PriceBox = ({ showTimer = true, showFeatures = false }: PriceBoxProps) => {
  const features = [
    "Receituário Funcional para Reduzir a Gordura no Fígado (Valor R$97,00)",
    "Guia Visual: Alimentos que Aumentam vs Reduzem a Gordura no Fígado (Valor R$59,00)",
    "Guia de Substituições Inteligentes para Esteatose (Valor R$47,00)",
    "Plano de 7 Dias para Apoiar a Redução de Gordura no Fígado (Valor R$39,00)",
    "Receituário de Desejos Sem Culpa (Amigos do Fígado) (Valor R$29,00)",
    "Mini Receituário: Pães e Massas que Não Sobrecarregam o Fígado (Valor R$31,00)",
    "Molhos e Patês Leves para Quem Tem Gordura no Fígado (Valor R$28,00)",
  ];
    ...
    <div className="bg-card rounded-xl md:rounded-2xl shadow-hero border border-border p-4 md:p-8 max-w-2xl mx-auto">
      ...
      <div className="text-center mb-2">
        <div className="inline-flex items-center gap-2 bg-success/10 text-success px-3 md:px-4 py-1.5 md:py-2 rounded-full">
          <span className="font-bold text-xs md:text-sm">Plano básico</span>
        </div>
      </div>
